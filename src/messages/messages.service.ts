...
import { ConversationByIdDto, ConversationDto } from './dto/get-conversation.dto';
import { createMessID } from 'src/ids';
...
export class MessagesService {
  ...
  async getAnConversation(conversationDto : ConversationDto){
    const conversation = await this.messagesRepository.find({where:{ roomId : conversationDto.roomId }})
    return sortBy(conversation,'timeSend');
  }

  async getAnConversationById(conversationByIdDto :  ConversationByIdDto){
    const conversation = await this.messagesRepository.find({where:{roomId : conversationByIdDto.roomId,
