...
import { createFriendID } from 'src/ids';

@Injectable()
export class FriendsService {
  constructor(@InjectRepository(Friend) private readonly friendsRepository : Repository<Friend>,){}
  async createFriend(createFriendDto : CreateFriendDto){
    
    const id = createFriendID();
  ...
  async getFriendReqList(userReceiveId : string){
    const friendReqList = await this.friendsRepository.find({where : {userReceiveEmail : userReceiveId ,acceptStatus :false}})
    return friendReqList;
  }
  async getFriendList(userId : string){
    const friendList = await this.friendsRepository.find({where : {userReceiveEmail : userId ,acceptStatus :true}})
    return friendList;
  }
